...
def _extract_welkom_paragraphs(selected_date: datetime, entry: dict, meded: dict, takenrooster_entries: list = None) -> list:
    ...
    try:
        ...
        for p in paras:
            ...
            # Skip hardcoded "Aanstaande..." paragraphs first (before other conditions)
            if p.strip().startswith('Aanstaande'):
        ...
        if takenrooster_entries:
            ...
            print(f"DEBUG: Found {len(upcoming)} upcoming services in next 7 days")
            for e in upcoming:
                d = e['date']
                ...
                aanstaande_line = f"Aanstaande {upcoming_day} {upcoming_date_str}, hoopt in de {online_prefix}{dienst_type}Eredienst in Amstelveen voor te gaan, {upcoming_predikant}. Aanvang is om 10:30 uur."
                print(f"DEBUG: Adding aanstaande line: '{aanstaande_line}'")
                result.append(aanstaande_line)
            ...
            try:
                ...
                mededelingen_data = {
                    ...
                    'sections': [
                        ...
                        _build_section('regionale', 'Regionale Mededelingen', 'Pengumuman Regional',
                                       meded.get('regionale_nl', ''), meded.get('regionale_id', '')),
                        _build_section('landelijke', 'Landelijke Mededelingen', 'Pengumuman Nasional',
...
@app.route('/campaign/ole/am-template', methods=['POST'])
@_password_required
def campaign_ole_am_template():
    """Generate AM OLE announcement email template with liturgie attachment reminder."""
    from sender_campaign import SenderCampaignGenerator
...
@app.route('/campaign/ole/am-combined', methods=['POST'])
@_password_required
def campaign_ole_am_combined():
    """Return AM OLE email text fields and a base64 .eml in one request."""
    from sender_campaign import SenderCampaignGenerator
    ...
    try:
        ...
        return jsonify({
            ...
            'eml_filename': f"GKIN OLE {iso_date}.eml",
            'note': 'Vergeet niet de liturgie als bijlage toe te voegen aan de e-mail. De QR code zit al in de .eml.'
        })
